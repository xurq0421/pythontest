...
class BaseData():
    #随机生成11位手机号
    def getPhone(self):
        start_num = ['131','132','134','135','158','159']
        #string.digits:0123456789   random.sample(list,5):list中随机取5数['5','8', '7', '6', '4']  ''.join是以什么格式分隔 可以是','.join就是按,分隔list
        end_num = ''.join(random.sample(string.digits,8))
        phone_num = random.choice(start_num) + end_num
        #print(phone_num)
        return phone_num
    ...
    def getName(self):
        firstName = ['赵', '钱', '孙', '李', '周', '吴', '郑', '王', '冯', '陈', '褚', '卫', '蒋', '沈', '韩', '杨', '朱', '秦', '尤', '许',
                ...
                '熊', '纪', '舒', '屈', '项', '祝', '董', '梁']
        lastName = ['的', '一', '是', '了', '我', '不', '人', '在', '他', '有', '这', '个', '上', '们', '来', '到', '时', '大', '地',
                ...
                '付', '阵', '玛', '批', '岛', '项', '休', '懂', '武', '革', '良', '恶', '恋', '委', '拥', '娜', '妙', '探', '呀',
        ...
        name = random.choice(firstName) + ''.join(random.sample(lastName,2))
        # print(name)
        return name

#随机生成身份证号
    def getId(self):
        datapath = os.path.dirname(os.path.realpath(__file__)) + '\\area_data.txt'
        fp = open(datapath,encoding='utf-8',mode='r')
        #read读回来的是Str类型的
        area_text = fp.read()
        #print(area_text)
        fp.close()
        #把str类型的转换成字典类型
        area_dict = eval(area_text)
        #print(type(area_text))

        area_list = []
        for key in area_dict.keys():
            area_list.append(key)
        year = random.randint(1965,1995)
        ...
        for month in months:
            if month == '02':
                ...
                if day < 10:
                    day = '0' + str(day)
        code = str(random.randint(0,999)) #年月日后3位数
        if len(code) == 1:
            code = '00' + code
        elif len(code) == 2:
            code = '0' + code
        id = random.choice(area_list) + str(year) + random.choice(months) + str(day) + code
        i = 0
        count = 0
    ...
    def getCardNo(self,cardBin,length):
        card_middle = []
        sum_old = 0
        sum_evn = 0
        for i in range(length - len(cardBin) - 1):
            card_middle.append(str(random.randint(0,9)))
        card = cardBin + ''.join(card_middle)
        for i in range(len(card)):
            if i%2 ==0:
                temp = int(card[-1-i])*2
                if temp >= 10:
                    temp = temp - 9
                sum_old = sum_old + temp
            else:
                sum_evn = sum_old + temp
        if (sum_old + sum_evn) % 10 ==0:
            return card + '0'
        else:
            return card + str(10 - (sum_evn + sum_old)%10)
        #print(card)
